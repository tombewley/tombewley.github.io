...
where $H(\pi)$ is the causal entropy [3] of the policy $\pi$. Let $\bar{c}\in\operatorname{IRL}_{\psi}\left(\pi_{E}\right)$ be the outcome of this optimisation and $\operatorname{RL}(\bar{c})=\operatorname{RL}\circ \operatorname{IRL}_{\psi}\left(\pi_{E}\right)$ be the policy learned with that cost function via reinforcement learning. In Appendix A.1, the authors prove that this policy can be equivalently written as

$$
\operatorname{RL}\circ\operatorname{IRL}_{\psi}\left(\pi_{E}\right)=\underset{\pi \in \Pi}{\operatorname{argmin}}\Big[\psi^{*}\left(\rho_{\pi}-\rho_{\pi_{E}}\right)-H(\pi)\Big]
$$

where $\psi^*$ is called the *convex conjugate* of $\psi$ and $\rho_{\pi}: \mathcal{S} \times \mathcal{A} \rightarrow \mathbb{R}$ is the *occupancy measure* of a policy $\pi$, defined as $\rho_{\pi}(s, a)=\pi(a | s) \sum_{t=0}^{\infty} \gamma^{t} P\left(s_{t}=s | \pi\right)$. The occupancy measure can be interpreted as the unnormalised distribution of state-action pairs that an agent encounters in the environment while executing its policy.
