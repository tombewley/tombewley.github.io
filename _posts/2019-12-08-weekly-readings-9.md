...
For a given task $T^i$ and target policy $\pi^i_{target}$, we can observe trajectories and store them in a dataset $\mathcal{D}^i$ where each element of $\mathcal{D}^i$ is a triple $(I_{t-1},I_{t},\mathbf{a}_t^i)$. $I_{t-1}$ and $I_t$ are consecutive high-dimensional observations and $\mathbf{a}_t^i$ is a vector corresponding to the action taken at time $t$.  
