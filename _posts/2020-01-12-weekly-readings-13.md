...
Nearby points are generated by an evolutionary process. Each member of the population $P$ is initiated with the target instance $x^*$. On each iteration, each instance $x$ is submitted to the black box $b$ which yields a prediction $b(x)$. Fitness is computed as
$$
\text{fitness}(x)=I_{b(x)=c}+(1-d(x,x^*))-I_{x=x^*}
$$
where $c\in\{0,1\}$ is one of the two classes, $d(x,x^*)\in[0,1]$ is a distance function between instances, and the final term ensures that $x^*$ itself is not retained. Both mutation and crossover are performed to maximise the fitness function. Finally, a selection of the best individuals is returned. The evolution process is completed for both classes, producing a balanced dataset. 
...
2. The inverse dynamics model is deployed to label a dataset of trajectories produced by the target policy $\pi^*$ (the maximum likelihood action is chosen for each transition). This newly-labelled dataset is then used to learn a policy approximation $\pi_\phi(a^i_t\vert s_t^i)$. For continuous and discrete actions respectively, the same structural assumptions are made as above (i.e. Gaussian or softmax).
3. Both learned models can be optionally improved with post-demonstration environmental interaction: following $\pi_\phi$ for a number of timesteps [and getting the actions relabeled by $\pi^*$?] produces datasets for refining first $\theta$, then $\phi$. 
